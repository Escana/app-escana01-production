-- Drop existing stats table if it exists
DROP TABLE IF EXISTS stats CASCADE;

-- Create stats table with proper structure
CREATE TABLE stats (
...
-- Create trigger to ensure daily stats exist when accessing visits
...
DROP POLICY IF EXISTS "Enable read access for all users" ON stats;
DROP POLICY IF EXISTS "Enable insert for authenticated users" ON stats;
DROP POLICY IF EXISTS "Enable update for authenticated users" ON stats;
...
-- Insert initial stats for today if they don't exist
INSERT INTO stats (
  date,
  total_visits,
  total_incidents,
